...
1. [Заполнить переменные окружения](#шаг-1-заполнить-переменные-окружения)
2. [Получить сертификат](#шаг-2-получить-сертификат)
3. [Найти службу приложения](#шаг-3-найти-службу-приложения)
4. [Положить файлы рядом с Compose](#шаг-4-положить-файлы-рядом-с-compose)
5. [Проверить Compose](#шаг-5-проверить-compose)
6. [Запустить или пересоздать Caddy](#шаг-6-запустить-или-пересоздать-caddy)
7. [Проверить HTTPS и редиректы](#шаг-7-проверить-https-и-редиректы)
...
Если у приложения уже есть свой `.env`, не затирайте его. Добавьте переменные из `.env.example` в существующий файл.
...
Откройте основной `docker-compose.yml` приложения и найдите службу, которая сейчас публикует порт наружу.
